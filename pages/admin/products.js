...
export default function ProductsAdmin(){
  ...
  async function create(e){
    ...
    if (res.ok){ const p = await res.json(); setProducts(prev=>[...prev,p]); setForm({name:'',slug:'',price:0,category:'men',image:'',description:''}) }
  }
      ...
      <div style={{display:'flex',gap:20}}>
        <div style={{flex:1}}>
          ...
          <form onSubmit={create}>
            ...
            <input placeholder="Price (FCFA)" type="number" value={form.price} onChange={e=>setForm({...form,price:Number(e.target.value)})} style={{width:'100%',padding:8,margin:'6px 0'}} />
